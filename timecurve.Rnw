    ...
    \item Sex, age in days, enrollment date, $p_i(0) =1$
      \begin{enumerate}
        \item look up 85 daily hazards in US rate tables: 
          $\lambda_i(a_{ij},  s_i, y_{ij})$
        \item increment $a$ and $y$ by one day
        \item   $\lambda_i(a_{ij},  s_i, y_{ij})$
          \item \ldots
        \item $\Lambda_i(t) = \sum_{s \le t} \lambda_i(s)$
      \end{enumerate}
    \item $S_i = e^{-\Lambda_i(t)}$
  \end{itemize}
\end{frame}

\begin{frame}{MCSA}
  \begin{itemize}
  \item Use the MSH fit to create rate tables for ND:dem, ND:death, dem:death
  \item Person-years estimates of future CMC/death, given start at 0, 
    start at 1, \ldots
  \item PY estimates of future amyloid/death, given start low, start mod, 
    start high

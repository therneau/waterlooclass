...
<<nafld0, echo=FALSE>>=
row.names(nafld1) <- NULL
row.names(nafld2) <- NULL
row.names(nafld3) <- NULL
...
\section{Data sets: counting process vs. timeline}
The survival package evolved along with a data set type I call a
``counting process'' data set. 
Most routines in the survival package can also deal with a ``timeline'' data set
(hopefully all routines, soon).
The \code{msm} and \code{mshmm} packages, which deal with interval censored
multi-state hazard models (MSH) accept only timeline style.
In a CP data set we have:
\begin{itemize}
  \item A subject identifier
  \item (time1, time2] mark out a time interval
  \item covariates apply over the interval (time1, time2]
  \item likewise for stratification variables
  \item optionally, current state
  \item outcome variables, e.g., status or state, decribe the change in state
    which occurs at time2, or censored= no change
\end{itemize}
In a timeline data set we have
\begin{itemize}
  \item A subject identifier
  \item A time variable
  \item Covariate values that are known or measured at that time
  \item Missing covariates are common
\end{itemize}

The basic premise for both is a data set where each person decribes a
physically possible path through time.  Here are three subjects from the
counting process nafld data set:
<<nprint, echo=FALSE>>=
...
Last value retained (last value carried forward or last observation carried
forward):  If a visit has partial information, e.g., BMI is in the model and
it wasn't measured on this visit, what do we use for a value?  The two
choices are to use the most recent available, or to extrapolate. LVCF is the
default in my code, part of the data set creation (\code{tmerge}) for CP
style data, at the analysis stage for TL style.

Immortal time bias. If any of these features of the data depend on the future,
bad things happen: $x_i(t)$, $Y_i(t)$, $N_i(t)$. Run away!
...
MASLD is currently responsible for almost 1/3 of
...
The \code{nafld} data set includes all patients with a MASLD
...
At each event time $t$ the AJ algorithm makes a matrix
\begin{align*}
  A(t) &= \left( \begin{array}{cccc}
    \square & \lambda_{12}(t) & \lambda_{13}(t) & \ldots \\
    \lambda_{21}(t) & \square & \lambda_{23}(t) & \ldots \\
    \lambda_{31}(t) & \lambda_{32} & \square & \ldots \\
    \vdots         & \vdots       & \vdots & \ddots 
   \end{array} \right) \\
  H(t) &= I + A \\
  \lambda_{jk} &= \frac{\sum_i dN_{ijk}(t)} {\sum_i Y_{ij}(t)}
\end{align*}
The diagonal of the $A$ matrix is such that each row sums to 0, for
$H$ each row sums to 1.
Then $$
p(t) = p(t_0) \prod_{t_0 \le s \le t} H(s)
$$
is the Aalen-Johansen.
 $p_j(t)$ = P(in state $j$ at time $t$), $0 \le p_j(t) \le 1$,
 $\sum_j p_j(t) =1$.

\begin{itemize}
  \item You can use any starting time $t_0$ that you like
  \item You can use any starting distribution you like
  \item survfit default
    \begin{itemize}
      \item $t_0$ = common starting time, if there is one, else the first transition time
      \item $p(t_0)$= emprical distribution at $t_0$ or $t_0-$
    \end{itemize}
  \item When using age scale the default is rarely useful
\end{itemize}


\section{Multistate hazard model}
...
Here is a non-parametric version

\begin{figure}
<<nafig2>>=
aj2 <- survfit(Surv(age1, age2, event) ~ male+ nafld, ndata, id=id, 
               istate=cstate, start.time=50, p0=c(1,0,0,0,0))
dim(aj2)
temp2 <- summary(aj2, rmean=100)$table
round(temp2[1:7,], 2)
rmat <- matrix(temp2[,'rmean'], ncol=4, byrow=TRUE)
colnames(rmat) <- c("Female, Normal", "Female, MAFLD", "Male, Normal",
                    "Male, MAFLD")
oldpar <- par(mar=c(5,9,1,1))
barplot(rmat[, c(2,1,3,4)], horiz=TRUE, xlim=c(0,50), 
        col=c("green","orange", "red", "darkred", "white"), las=1,
             xlab="Years in state, from age 50")
@ 
 \caption{Non parametric estimate using the AJ}
  \label{nafigaj}
\end{figure}

Where do the standard errors come from?
<<se>>=
ijmat <- resid(aj2, type="sojourn", time=c(50, 100))
dim(ndata)
@ 
\end{document}

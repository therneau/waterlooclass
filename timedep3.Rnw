...
\begin{frame}{The problem with ``baseline'' CMC}
  \begin{itemize}
    \item Mean fu is 8 years
    \item Create the curve for a 65 year old, low baseline CMC
      \begin{itemize}
        \item by age 73 all new subjects
        \item by age 81 again new
        \item real CMC drifts up, current cohort CMC does not
      \end{itemize}
    \item The usual KM will be biased, too few events
  \end{itemize}
\end{frame}

\begin{frame} {Expected Survival (survexp)}
  \begin{itemize}
    \item Cohort of 85 subjects, what is their expected 10 year survival?
    \item Sex, age in days, enrollment date, $p_i(0) =1$
      \begin{enumerate}
        \item look up 85 daily hazards in US rate tables
        \item $h(t) = \sum_{i=1}^{85}  p_i(t)h_i(t)/ \sum p_i(t)$
        \item $p_i(t)= p_i(t) exp(-h_i(t))$
        \item move ahead one day
      \end{enumerate}
    \item $S = e^{h_1} e^{h_2} \ldots$
  \end{itemize}
\end{frame}

\begin{frame}{MCSA}
  \begin{itemize}
  \item Use modified MSH to create a 3 column rate table by age, sex, APOE, 
    amyloid, CMC (ND:dem, ND:death, dem:death)
  \item AJ estimates of future CMC/death, given start at 0, start at 1, \ldots
  \item AJ estimates of future amyloid/death, given start low, start mod, 
    start high
  \item starting point $\pi_i$ of state * CMC * amyloid
   \begin{enumerate}
     \item take weighted average across $pi_i$ of the hazard, for ND:dem,
       ND:death, dem:death
     \item plug these into an H(t) matrix
     \item Update the $\pi$ vectors for each subject, move ahead 1 year
   \end{enumerate}
   \item $p(t)= e^{H_1} e^{H_2} \ldots$
   \item ``distribution of recruited individuals''
  \end{itemize}
\end{frame}
...
legend(0, 1, c("KM, baseline covariate", "Cox, baseline covariate", 
                 "Cox, TD covariate"), lty=1, col=1:3,
       lwd=2, bty='n')
@
\end{frame}
...
\begin{frame}{Multistate curves}
\begin{itemize}
  \item Ordinary Cox model: specify the covariates
  \item AJ: specify starting time and starting state
  \item MSH: specify covariates, starting time, starting state
\end{itemize}
\end{frame}
...
plot(aj3b, col=c(2,3,4,1), xmax=12, lwd =2, xlab="Years from randomization",
     ylab= "P(state)")
legend(4,1, c("low", "mod", "high", "death"), lwd=2, lty=1, col=c(2,3,4,1),
       bty='n')
...
\begin{frame}[fragile]
<<echo=TRUE>>=
aj3  <- survfit(Surv(year1, year2, bstate) ~ 1, pdata, 
                istate=bili3, id=id)
...
\begin{frame}{Math}
  The death curve for group 1 = ``normal'' will have an increment at time $t$ of
  \begin{itemize}
...
\begin{frame}{Hazard models}
  ...
  \begin{itemize}
    ...
    \item Do we want covariates for the within bilirubin transitions?
  \end{itemize}
\end{frame}
